...
const GITHUB_REPO_FALLBACK = "simontigers/dsh-wecom-notice";
...
/** 本插件自身版本 + profile 依赖声明（安装来源github:/link:/registry）。 */
async function resolveInstallInfo() {
  const info = { mode: "unknown", spec: "", repoSlug: GITHUB_REPO_FALLBACK, currentTag: "", version: "" };
  try {
    const pkg = JSON.parse(await readFile(new URL("../package.json", import.meta.url), "utf8"));
    info.version = String(pkg.version ?? "");
  } catch { /* 忽略 */ }
  try {
...
function wireEvents(ctx, cfgRef, disposers) {
  ...
  const scheduleTurnPush = (session, event) => {
    if (cfgRef.config.enabled === false) return;
    const reason = isPlainObject(event?.data?.reason) ? event.data.reason : {};
    const kind = typeof reason.kind === "string" && reason.kind !== "" ? reason.kind : "unknown";
    const completed = kind === "completed";
  ...
  disposers.push(hostCtx.on("session/event", (...args) => {
    try {
      ...
      if (event.type === "turn/end") {
        ...
        if (cfgRef.config.enabled !== false && cfgRef.config.events.approval) void pushApproval(session, event, cfgRef.config, log);
      }
    } catch (error) {
  ...
  disposers.push(hostCtx.on("agent/error", (payload = {}) => {
    try {
      if (cfgRef.config.enabled === false || !cfgRef.config.events.agentError) return;
      const agentSession = payload?.agent?.session;
  ...
  return {
    ...
    async checkUpdate() {
      ...
      if (info.mode === "github") {
        ...
        if (latest !== null) {
          return {
            mode: "github", repoSlug: info.repoSlug, currentTag: info.currentTag, version: info.version,
            latest, upToDate: !isNewerVersion(latest, info.currentTag),
          };
        }
        // 主机侧网络不通 → 返回 repoSlug/currentTag，让浏览器端兜底拉取
        return { mode: "github", repoSlug: info.repoSlug, currentTag: info.currentTag, version: info.version, latest: null };
      }
      return { mode: info.mode, spec: info.spec, version: info.version };
    },
  };
}
...
export const apply = (ctx) => {
  ...
  // 停用（enabled=false）时抛错，让调用方走各自的降级路径，且不向企业微信发送任何消息。
  ctx.provide("wecomNotice", {
    async send({ title, content }) {
      if (cfgRef.config.enabled === false) {
        throw new Error("企业微信通知通道已停用（enabled=false）");
      }
